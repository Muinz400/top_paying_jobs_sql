...
/* Question: What are the top-paying data analyst jobs, and what skills are 
required?** 
...
SELECT COUNT(job_postings_fact.job_id) AS total_jobs,
       job_title_short,
       skills_dim.skills
FROM job_postings_fact
JOIN skills_job_dim 
...
SELECT AVG(job_postings_fact.salary_year_avg) AS avg_salary,
       skills_dim.skills
FROM job_postings_fact
...
**Answer: What are the most optimal skills to learn (aka it’s in high 
...
FROM skills_demand
JOIN highest_avg_salary
ON skills_demand.skill_id = highest_avg_salary.skill_id
